...
| **Tipo**: |Primário|
| **Descrição**: | As funções disponibilizadas são: Cadastro (Compra) de Jogador, Buscar Jogador, Alteração de Informações de Jogador, Listagem de Jogador e Exclusão (Venda) de Jogador. As funções de Compra, Venda e Alteração de Informações de Jogador são registradas no Histórico de Auditoria.|
